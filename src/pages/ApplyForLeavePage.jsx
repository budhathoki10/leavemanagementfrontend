...
const ApplyForLeave = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (res.data && res.data.data) {
        setStudentName(res.data.data.studentname || "Student");
      }
    } catch (err) {
  ...
  const getModuleRemainingLeaves = (moduleId) => {
    if (!leaveStats || !leaveStats.moduleWiseStats) return 8;

    const moduleStat = leaveStats.moduleWiseStats.find(
      (stat) => stat.module._id === moduleId
    );

    return moduleStat ? moduleStat.remainingLeaves : 8;
  };
    ...
    try {
      ...
      if (pictures.length > 0) {
        ...
        response = await axios.post(
          "https://leave-management-backend-kkk.onrender.com/api/task/create",
          formData,
          {
            headers: {
